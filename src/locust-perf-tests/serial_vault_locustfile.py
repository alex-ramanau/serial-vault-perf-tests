...
class SerialVaultUser(HttpUser):
    wait_time = between(1, 2)  # Wait between 1 and 2 seconds between tasks

    @task(50)
    def request_id(self):
        headers = {
            "API-KEY": os.environ.get("API_KEY", "default-api-key")
        }
        self.client.post("/v1/request-id", headers=headers, json={})

    @task(1)
    def get_metrics(self):
        self.client.get("/_status/metrics")
